...
def login(browser, email=None, save_path="state.json"):
    ...
    if choice == "1":
        ...
        logger.info(f"邮箱内容: {email_input.input_value()}")
        logger.info(f"当前URL: {shop_page.url}")
        ...
        logger.info(f"Continue 后 URL: {shop_page.url}")
        logger.info(f"Continue 后标题: {shop_page.title()}")
        ...
        while True:
            ...
            if (
                ...
                logger.warning("\n⚠️ 检测到 Cloudflare 验证")
                logger.info("请手动点击 Verify you are human")
            ...
            try:
                ...
                logger.info("✅ 找到 OTP 按钮")

                btn.click()

                logger.info("✅ OTP 按钮点击成功")
                ...
                logger.info("\n⏳ OTP 按钮还没出现")
                logger.info(f"当前 URL: {shop_page.url}")
        ...
        logger.info(f"登录后URL: {shop_page.url}")
        ...
        if save.lower() == "y":
            ...
            logger.info(f"✅ state 已保存到 {save_path}")
        else:
            logger.warning("❌ 未保存 state")
    ...
    # 直接使用邮箱登录（不弹新窗口）
        ...
        logger.info("=" * 60)
        logger.info("请在页面中手动完成：")
        logger.info("1. Cloudflare 验证")
        logger.info("2. 输入验证码并完成登录")
        logger.info("完成后按 Enter 继续...")
        logger.info("=" * 60)
        input()
        ...
        if "Couldn't sign you in" in body_text or "check your connection" in body_text:
            logger.warning("⚠️ 登录失败：验证码无效或链接过期")
